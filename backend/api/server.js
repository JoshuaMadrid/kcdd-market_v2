...
// NOTE: Uses SUPABASE_SECRET_KEY — matches backend/api/.env and CLAUDE.md convention.
 ...
 * - requestId: string
 *
 * donorId is resolved server-side from Clerk JWT — never trusted from client.
...
app.post('/api/payments/create-intent', clerkAuth, async (req, res) => {
  try {
    ...
    // Security: resolve donorId from the verified Clerk JWT, not from the request body
    ...
    // Verify request exists and get organization's Stripe account + canonical amount from DB
    ...
    // Use the DB amount — never the client-supplied value
    ...
    const { error: idempotencyError } = await supabase
      .from('stripe_events')
      .insert({
        event_id: `intent_${requestId}_${donorId}`,
        event_type: 'payment_intent.create',
        payload: { requestId, donorId },
      })

    if (idempotencyError && idempotencyError.code !== '23505') {
      // Non-uniqueness error — unexpected
    ...
    const { error: txError } = await supabase.from('payment_transactions').insert({
      ...
      organization_id: org?.id || request.organization_id,
...
async function handlePaymentSucceeded(paymentIntent) {
  const { requestId, campaignId, organizationId, organizationUserId, donorId } = paymentIntent.metadata
