  ...
  return (
    <Card className="sticky top-6 w-[340px] overflow-hidden rounded-xl border-[#e5e5e5] shadow-sm">
      ...
      <div className="space-y-5 p-6">
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {causeAreas.map(({ cause_area }) => (
                <Badge key={cause_area.id} variant="outline" className="text-xs">
                  {cause_area.name}
                </Badge>
              ))}
            </div>
        ...
        {/* Support CTA — hidden for org owners (no self-support) */}
        {!isOwner && showSupportButton && (
          <Link to={`/requests?organization=${organization.id}`} className="block pt-2">
            <Button
              className="h-11 w-full rounded-full bg-[#ea580c] text-base font-medium text-white hover:bg-[#dc4c06]"
