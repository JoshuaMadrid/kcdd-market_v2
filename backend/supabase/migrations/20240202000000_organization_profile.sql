...
-- Adds fields for enhanced organization profiles, updates, and team members
...
-- Links organizations to identity categories (populations served)
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_organization_populations_org_id
ON organization_populations(organization_id);

-- ============================================
-- ORGANIZATION UPDATES TABLE
-- News and updates posted by organizations
-- ============================================
...
-- Index for faster lookups
CREATE INDEX IF NOT EXISTS idx_organization_updates_org_id
ON organization_updates(organization_id);

CREATE INDEX IF NOT EXISTS idx_organization_updates_published
ON organization_updates(organization_id, is_published, created_at DESC);

-- ============================================
-- ORGANIZATION TEAM MEMBERS TABLE
-- Team members displayed on organization profiles
-- ============================================
...
-- Index for faster lookups
CREATE INDEX IF NOT EXISTS idx_organization_team_members_org_id
...
CREATE POLICY "Anyone can view organization populations"
ON organization_populations FOR SELECT
USING (true);

CREATE POLICY "Anyone can view published updates"
ON organization_updates FOR SELECT
...
-- Organization owners can manage their own data
CREATE POLICY "Org owners can manage populations"
ON organization_populations FOR ALL
USING (
  organization_id IN (
...
CREATE POLICY "Org owners can manage updates"
ON organization_updates FOR ALL
USING (
  organization_id IN (
    SELECT id FROM organizations WHERE user_id = auth.uid()
  )
